...
def test_Band_and_Time_Limited_White_Noise_Burst(
    ...
    for seed in range(duplication):
        ...
        # h_plus_, h_cross_ = lalWhiteNoiseBursts(
        #     duration=time_envelope,
        #     frequency=frequency,
        #     bandwidth=bandwidth,
        #     eccentricity=eccentricity,
        ...
        #     int_hdot_squared=int_hdot_squared,
        #     delta_t=1 / sample_rate,
        #     rng=lal.gsl_rng("taus2", seed),
        # )
        # hcross_.append(h_cross_.data.data)
        # hplus_.append(h_plus_.data.data)

    hcross = np.vstack(hcross)
    hplus = np.vstack(hplus)
    # hcross_ = np.vstack(hcross_)
    # hplus_ = np.vstack(hplus_)
    ...
    if lal_samples < ml4gw_samples:
        ...
        # plus, cross = hplus_[:,start:stop], hcross_[:,start:stop]
